...
function updateToasts(toasts: ToasterToast[]) {
  toastState.toasts = toasts;
  listeners.forEach(listener => listener(toasts));
}

export function useToast() {
  function toast({ title, description, variant, duration = 5000, action }: ToastOptions) {
    const id = Math.random().toString(36).substring(2, 9);
    
    const update = toastState.toasts.concat({
      id,
      title,
      description,
      variant,
      ...
      onOpenChange: open => {
        if (!open) {
          ...
          setTimeout(() => {
            updateToasts(toastState.toasts.filter(t => t.id !== id));
          }, TOAST_REMOVE_DELAY);
        }
      }
    }).slice(0, TOAST_LIMIT);
