...
function readData() {
  if (!fs.existsSync(DATA_PATH)) {
    return { users: [], groups: [], sessions: {}, resetTokens: {} };
  }
...
export function createResetToken(email, token) {
  const data = readData();
  data.resetTokens[token] = email;
  writeData(data);
}

export function getEmailByResetToken(token) {
  const data = readData();
  return data.resetTokens[token] || null;
}
